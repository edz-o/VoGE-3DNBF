...
__global__ void RayTraceFineVogeKernel(
    ...
    const int H,
    const int W,
    int32_t* point_idxs, // [N, W, H, K]
    float* total_len, // [N, W, H, K]
    float* total_act, // [N, W, H, K]
    float* total_dsd // [N, W, H, K]
){
    ...
    for (int pid = tid; pid < num_pixels; pid += num_threads) {
        ...
        const int ray_idx = n * W * H + yi * W + xi;
        ...
        for (int m = 0; m < M; ++m) {
            const int idx_point = bin_points[n * BW * BH * M + by * BW * M + bx * M + m];
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> RayTraceFineVoge(
    ...
    const at::Tensor& bin_points, // (N, BH, BW, M)
    const float thr_act, // -log(thr + 1e-10)
    const int bin_size,
    const int K
) {
    ...
    at::Tensor total_len = at::full({N, H, W, K}, 1e10, float_opts);
    at::Tensor total_act = at::full({N, H, W, K}, 1e10, float_opts);
    at::Tensor total_dsd = at::full({N, H, W, K}, 0, float_opts);
...
__global__ void RayTraceFineVogeBackwardKernel(
    ...
    const float* mus, // (P, 3)
    const float* rays, // (N, H, W, 3)
    const int32_t* point_idxs, // (N, H, W, k)
    const int K,
    const int N,
    const int H,
    const int W,
    const float* grad_len, // (N, H, W, K)
    const float* grad_act, // (N, H, W, K)
    const float* grad_dsd, // (N, H, W, K, 3)
        
    float* grad_ray, // (N, H, W, 3)
    float* grad_mus, // (P, 3)
    ...
    for (int pid = tid; pid < N * H * W * K; pid += num_threads) {
        ...
        const int ray_idx = n * H * W + yi * W + xi;
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> RayTraceFineVogeBackward(
    ...
    at::Tensor grad_ray_out = at::zeros({N, H, W, 3}, float_opts);
