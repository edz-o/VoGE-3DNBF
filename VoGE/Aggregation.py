...
def get_cross_activation(sel_length, sel_dsd):
    ...
    :param ray_grids: [n, k, 3]
    :param sel_length: [n, k, M]
    :param sel_dsd: [n, k, M] -> 2 * K.T @ iSigma_k @ K
    :return: cross_activation [n, k, M (_m), M (_k)]:
                     (l_m - l_k) * (K.T @ iSigma_k @ K) ^ .5
    """
    n, k, m = sel_length.shape

    # [n, k, M(_m), 1] - [n, k, 1, M(_k)] -> [n, k, M(_m), M(_k)]
    # [n, k, M(_m), M(_k)] * [n, k, 1, M(_k)]
    cross_activation = (sel_length.unsqueeze(3) - sel_length.unsqueeze(2)) * (sel_dsd.view(n, k, 1, m) + 1e-10).pow(.5) 
...
def assign2weight(sel_activation, cross_activation, occupation_weight=1.):
    ...
    :param sel_activation: [N, k, M]
    :param cross_activation: [N, k, M, M]
    :param occupation_weight: float, the weight when calculating mutual occlusion.

    :return: (weight)
    weight: [N, k, M]
    """
    # [N, k, 1, M(_k)] * erf( [N, k, M (_m), M (_k)] )
    density_dist = torch.exp(-sel_activation.unsqueeze(2)) * ((torch.erf(cross_activation) + 1) / 2)

    # Sum_{k=1}^(M) -> Sum_{k=1}^(M, m != k)
    # [N, k, M (_m), M (_k)].sum(2) - [N, k, M (_m)]
    density_weight = torch.exp(-(torch.sum(density_dist, dim=3)) * occupation_weight)
...
def aggregation(sel_idx: torch.Tensor, sel_act: torch.Tensor, sel_len: torch.Tensor, sel_dsd: torch.Tensor, occupation_weight: float=1.):
    """
    :param sel_idx: [N, w, h, M] vertices indexs along rays
    :param sel_act: [N, w, h, M] hit activations along rays
    :param sel_len: [N, w, h, M] hit length along rays
    :param sel_dsd: [N, w, h, M] eaualvalent isigma along the ray -> D.T @ isigmas @ D
    ...
    :return: weight matrix: [N, w, h, M]: torch.float32 vertices weight at each location
             index matrix: [N, w, h, M]: torch.int
             valid_num: [N, w, h]: torch.int
    """
    bs = sel_idx.shape[0]
    M = sel_idx.shape[-1]
    shape_ = sel_idx.shape[1:-1]
    reshape_foo = lambda x, tar_shape=shape_: x.view(bs, *tar_shape, *dict() if len(x.shape) == 1 else (-1, ))

    # [N, k', M, M]
    cross_activation = get_cross_activation(sel_length=sel_len.view(bs, -1, M), sel_dsd=sel_dsd.view(bs, -1, M))

    # [N, k', M]
    get_weight = assign2weight(sel_activation=sel_act.view(bs, -1, M), cross_activation=cross_activation, occupation_weight=occupation_weight)
...
def expend_sigma(sigma, rotation_matrix=None):
    ...
    if len(sigma.shape) == 4:
        if sigma.shape[2] == 3 and sigma.shape[3] == 3:
            return sigma
        else:
