...
def ray_tracing(transforms, points, isigmas, rays, image_size, thr: float, n_assign: int,
    ...
    if max_points_per_bin == -1:
        ...
        # i.e. bin_points: N, point_idx_size, point_idx_size, P
        N = rays.shape[0]
        P = int(points.shape[0]/rays.shape[0])
        pixels_to_points_idx = [torch.arange(P).view(1, 1, 1, -1) \
                                    .expand(-1, point_idx_size[0], point_idx_size[1],-1) \
                                    .type(torch.int32).contiguous().to(points.device) \
                                    #.repeat(rays.shape[0], 1, 1, 1) 
                                    + n*P for n in range(N)]
        pixels_to_points_idx = torch.cat(pixels_to_points_idx, dim=0)
    else:
...
# mus: kernel_center, isigmas: kernel_shape
def ray_tracing_fine(mus, isigmas, rays, bin_points, thr, bin_size, n_assign, inf=1e10):
    ...
    assert rays.shape[0] == bin_points.shape[0]
